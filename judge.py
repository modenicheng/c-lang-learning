...
for idx, inp in enumerate(input_data):
    ...
    if result.stdout == expected_output:
        console.log(
            f"[green]Test case {idx + 1} passed! (Took {readable_perf_time(execution_time)})[/green]"
        )
    else:
        console.log(
            f"[red]Test case {idx + 1} failed! (Took {readable_perf_time(execution_time)})[/red]"
        )
...
for idx, result in enumerate(results):
    ...
    results_table.add_row(
        ...
        readable_perf_time(execution_times[idx])
        if idx < len(execution_times) else "N/A",  # 显示执行时间
        style=row_style)
