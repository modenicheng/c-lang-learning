...
用 n 表示待拆分的正整数，用 m 表示从 n 中拆出的最大正整数，则计算对正整数 n
共有多少种拆分方法可以下列递归公式： 0     ( 当 n < 1 或 m < 1 时 ) 1 ( 当 n = 1
或 m = 1 时 ) count(n,m)= count(n, n)                   ( 当 n < m 时 ) count(n,
m-1) + 1             ( 当 n=m 时 ) count(n-m, m) + count(n, m-1) ( 其他情况 )
...
参数说明：n 待拆分的正整数，m 表示从 n
中拆出的最大正整数；函数返回值是拆分方法数。
